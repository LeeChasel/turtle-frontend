    ...
    <form className="overflow-x-auto border square border-dark " style={{backgroundColor: "white"}} onSubmit={submit}>
      ...
      <tbody>
        <tr>
          ...
          <td><input type="text" className="w-full max-w-xs input input-bordered" ref={oldPasswordRef}/></td>
        </tr>
        <tr>
          <th style={{textAlign: 'right'}}>請輸入新密碼</th>
          <td><input type="text"className="w-full max-w-xs input input-bordered" ref={newPasswordRef}/></td>
        </tr>
        <tr>
          <th style={{textAlign: 'right'}}>再次輸入密碼</th>
          <td><input type="text"className="w-full max-w-xs input input-bordered" ref={checkNewpasswordRef}/></td>
        </tr>
      </tbody>
    </table>
    
    <p className="text-center">
      <button className="btn" disabled = {isSending}>確認</button>
      <button className="btn" type="button" onClick={cancel}>取消</button>
    </p>
  </form>
  )
}
