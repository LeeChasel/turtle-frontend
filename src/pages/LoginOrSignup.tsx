...
function LoginOrSignup() {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<TLogin>();
  ...
  return (
    <div className="flex justify-center pt-10">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-center w-1/3 gap-5 p-10 bg-gray-200">
        <div className="flex items-center gap-3">
          <label>帳號</label>
          <input type="text" {...register("email", { required: true })} className="w-full max-w-xs input"/>
          {errors.email && <span className="text-error">請輸入帳號</span>}
        </div>
        <div className="flex items-center gap-3">
          <label>密碼</label>
          <input type="password" {...register("password", { required: true })} className="w-full max-w-xs input"/>
