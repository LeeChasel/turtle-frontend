  ...
  return (
    ...
    <nav>
      <ul className="flex items-center gap-6 ">
        ...
        <li><Link to="/newProducts" className='relative p-1 text-4xl font-normal rounded-md text-sky-50 top-1 hover:bg-gray-700'>新上市</Link></li>
      </ul>
    </nav>
    ...
    <div className='flex items-center gap-7'>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className='flex justify-between gap-3'>            
          <UserStatusLink/>
          <Link to="#" className='text-2xl font-normal rounded-md text-sky-50 hover:bg-gray-700'>訂單查詢</Link>
        </div>
      </div>
    </div>
  </header>
...
function UserStatusLink() {
  ...
  return (
    <Link to={linkData.to} className="text-2xl font-normal rounded-md text-sky-50 hover:bg-gray-700">{linkData.text}</Link>
  )
}
...
function SearchBar() {
  ...
  return (
    <form onSubmit={handleSearch} className='flex w-full'>
      <input type="text" placeholder='Search' className="w-full h-8 indent-2 rounded-l-md"/>
