...
function InfoContainer() {
  ...
  const changeImage = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    const newVariation = product.variation?.find(item => item.variationName === variationName && item.variationSpec === variationSpec);
    if (!newVariation) {
  ...
  return (  
    <div className="flex mb-[108px]">
      <div className="px-10 basis-1/2">
        <ImageGallery items={galleryImages} showPlayButton={false} showFullscreenButton={false} ref={galleryRef} />
      </div>
      <div className="flex flex-col basis-1/2 pl-[105px]">
        ...
        <section className="inline-block px-4 py-2 mb-3 text-4xl text-white bg-blue-500 rounded-full w-fit">${variation.currentPrice}</section>
        <div className="flex flex-col gap-3 mb-[34px] p-3 border border-black rounded-lg shadow border-opacity-20">
          ...
          <select className="w-full select select-bordered" defaultValue={variation.variationName + '-' + variation.variationSpec} onChange={changeImage}>
            {product.variation?.map((item, index) => {
              const optionName = item.variationName + " - " + item.variationSpec;
              const optionValue = item.variationName + "-" + item.variationSpec + "-" + index;
              const available = item.available;
              return (
                <option key={index} value={optionValue} disabled={available ? false : true} className={available ? "" : "bg-gray-300"}>{optionName}</option>
              )
            })}
          </select>
        </div>
